      ...
      <div className='row g-0'>
        ...
        <div className='col-md-6'>
          <div className='card-body p-4 h-100 d-flex flex-column justify-content-between'>
            <h4 className='card-title text-start'>{pizza.name ?(pizza.name) : ''}</h4>
            <p className='card-text text-start'>{pizza.desc}</p>
            <p className='card-text text-start'>{pizza.ingredients ? pizza.ingredients.map(i => (<li key={i}>🍕 {(i)}</li>)) : null}</p>
            <div className='d-flex justify-content-evenly ms-3'>
              <h5 className='card-text d-flex align-items-center'>Precio: ${pizza.price ?(pizza.price) : '0'}</h5>
